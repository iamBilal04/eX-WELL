...
function Test() {
  const { predictions, setPredictions } = useFilePath();
 ...
 const navigate = useNavigate();
  ...
  return (
    <>
    {loading? "Loading...": 
    <div className="sm:w-[32rem] shadow-blue-100 mx-auto my-10 overflow-hidden rounded-2xl bg-white shadow-lg sm:max-w-lg items-center m-5">
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border-4 border-dashed px-4 py-10">
          ...
          <p className="mt-4 text-center text-xl font-medium text-gray-800">
            ...
            <label className="shadow-blue-100 mt-2 block rounded-full border bg-white px-4 py-0.5 font-normal text-blue-500 shadow hover:bg-blue-50">
              <input className="" type="file" name="file" onChange={(e)=>setFile(e.target.files[0])} />
              browse
            </label>
          </p>
        </div>

        <button className="mt-4 rounded-full bg-blue-600 px-10 py-2 font-semibold text-white hover:bg-blue-400" type="submit" onClick={prediction}>
          Submit
        </button>
      </div>
    </div>
